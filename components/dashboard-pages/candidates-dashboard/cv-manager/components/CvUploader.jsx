    ...
    const cvManagerHandler = (e) => {
        ...
        if (!isExist) {
            if (checkFileTypes(data)) {
                ...
                setError("نقبل فقط الملفات من نوع  (.doc, .docx, .pdf) file");
            }
        } else {
            setError("الملف موجود بالفعل");
        }
    };
            ...
            <div className="uploading-resume">
                <div className="uploadButton">
                    ...
                    <label className="cv-uploadButton" htmlFor="upload">
                        <span className="title">قم بأفلات الملف هنا لرفعه</span>
                        <span className="text">
                            اقصي حجم للملف هو  (Max 5Mb) و يسمح بانواع الملفات هذه فقط (.doc, .docx, .pdf)
                        </span>
